...
def validate_dfa(automaton: Automaton):
    assert automaton._type == AutomatonType.DFA, f"validate_dfa: Cannot validate automaton of type {automaton.type}"
    start_state_count = 0
    ...
    assert automaton._alphabet != None, "DFA must have an alphabet!"
    # No epsilon transition
    epsilon_transitions = [x.to_string() for x in automaton._transitions if "eps" in x.symbols]
    ...
    transitions_with_invalid_symbol = [x.to_string() for x in automaton._transitions if not x.symbols.issubset(automaton._alphabet)]
    ...
    missing_transitions = []
    for stateName, stateTransitions in automaton._state_out_transitions_map.items():
